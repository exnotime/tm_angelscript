...
angelscript_lib_dir = {"external_libs/tm_angelscript_lib/lib"}
angelscript_include_dir = {"external_libs/tm_angelscript_lib/include"}


filter "platforms:Win64"
    targetdir "%TM_BIN_DIR%/plugins" --Build directly into the plugin dir
    defines { "TM_OS_WINDOWS", "_CRT_SECURE_NO_WARNINGS" }
    includedirs { angelscript_include_dir,  "%TM_SDK_DIR%/headers" }
    staticruntime "On"
    architecture "x64"
    prebuildcommands {
        "if not defined TM_SDK_DIR (echo ERROR: Environment variable TM_SDK_DIR must be set)"
    }
    libdirs { angelscript_lib_dir,  "%TM_SDK_DIR%/lib/" .. _ACTION .. "/%{cfg.buildcfg}"}
    disablewarnings {
...
angelscript_debug_libs = {"angelscript64d.lib"}
angelscript_libs = {"angelscript64.lib"}

project "tm_angelscript"
    location "build/tm_angelscript"
    targetname "tm_angelscript"
    kind "SharedLib"
    language "C++"
